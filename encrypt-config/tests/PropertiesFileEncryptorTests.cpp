...
bool check_encryption(const PropertiesFile& test_file, const std::string& property_name, size_t original_value_length) {
...
bool operator!=(const PropertiesFile::Line& left, const PropertiesFile::Line& right) { return !(left == right); }
bool operator==(const PropertiesFile& left, const PropertiesFile& right) { return std::ranges::equal(left, right); }
bool operator!=(const PropertiesFile& left, const PropertiesFile& right) { return !(left == right); }
}  // namespace org::apache::nifi::minifi
...
TEST_CASE("PropertiesFileEncryptor can encrypt the sensitive properties", "[encrypt-config][encryptSensitivePropertiesInFile]") {
  utils::crypto::Bytes KEY = utils::string::from_base64("6q9u8LEDy1/CdmSBm8oSqPS/Ds5UOD2nRouP8yUoK10=");

  SECTION("default properties") {
    const auto test_file_path = std::filesystem::path("resources") / "minifi.properties";
    const auto sensitive_properties = minifi::encrypt_config::getSensitiveProperties(test_file_path);
    ...
    uint32_t initial_num_properties_encrypted = encryptSensitivePropertiesInFile(test_file, sensitive_properties, KEY);
    ...
    SECTION("calling encryptSensitiveValuesInMinifiProperties a second time does nothing") {
      ...
      uint32_t num_properties_encrypted = encryptSensitivePropertiesInFile(test_file, sensitive_properties, KEY);
  ...
  SECTION("with additional properties") {
    const auto test_file_path = std::filesystem::path("resources") / "with-additional-sensitive-props.minifi.properties";
    const auto sensitive_properties = minifi::encrypt_config::getSensitiveProperties(test_file_path);
    PropertiesFile test_file{std::ifstream{test_file_path}};
