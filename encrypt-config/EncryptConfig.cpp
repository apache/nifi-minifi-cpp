...
utils::crypto::Bytes EncryptConfig::getEncryptionKey() const {
  ...
  if (key_from_bootstrap_file && !key_from_bootstrap_file->empty()) {
    std::string base64_decoded_key = base64DecodeAndValidateKey(*key_from_bootstrap_file);
    ...
    utils::crypto::Bytes encryption_key = utils::crypto::randomBytes(ENCRYPTION_KEY_SIZE);
...
std::string EncryptConfig::base64DecodeAndValidateKey(const std::string& key) const {
