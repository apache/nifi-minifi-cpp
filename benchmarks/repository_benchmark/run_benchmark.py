...
def write_properties_file(properties: dict[str, str], path: str) -> None:
    with open(path, "w") as properties_file:
        properties_file.writelines(f"{key}={value}\n" for key, value in properties.items())
...
def generate_single_input(input_dir: str, file_size: int, index: int) -> None:
    ...
    with open(tmp_path, "wb") as input_file:
        ...
        while remaining:
            ...
            input_file.write(os.urandom(chunk_size))
...
def metrics_collector_loop(
    stop_event: threading.Event, container, samples: list, interval: float, start: float
) -> None:
...
def parse_docker_timestamp(value: str) -> datetime:
    # Docker timestamps use RFC 3339 with nanosecond precision and a trailing 'Z', which datetime.fromisoformat rejects on Python < 3.11.
...
def wait_for_minifi_to_start(container, timeout: float = 30.0) -> None:
    ...
    since = parse_docker_timestamp(container.attrs["Created"])
    while time.monotonic() < deadline:
        ...
        if "MiNiFi started" in logs:
            return
        time.sleep(0.5)
    raise RuntimeError(f"Timed out after {timeout}s waiting for MiNiFi to start (container status: {container.status})")


def build_extra_attributes(count: int, size: int) -> dict[str, str]:
    alphabet = string.ascii_letters + string.digits
    return {f"bench_attr_{index}": "".join(random.choices(alphabet, k=size)) for index in range(count)}
...
def write_config_yml(args: argparse.Namespace, work_dir: str) -> None:
    ...
    extra_attributes = build_extra_attributes(args.attribute_count, args.attribute_size)
    if args.input_file_generation_type == InputGenerationType.TIMED_GENERATEFLOWFILE:
        ...
        flow_config = flow_config_template.render(
            generate_file_interval=args.input_interval,
        ...
        flow_config = flow_config_template.render(
            get_file_interval=args.input_interval, extra_attributes=extra_attributes
        )
    with open(os.path.join(work_dir, "config.yml"), "w") as config_file:
        config_file.write(flow_config)


def create_minifi_container(
    args: argparse.Namespace, work_dir: str, input_dir: str
...
def wait_for_flow_files_to_be_processed(container, expected_count: int, timeout: float = 300.0) -> None:
    ...
    while True:
        ...
        if container.status != "running":
            raise RuntimeError(
                f"Container exited before processing {expected_count} flow files (status: {container.status})"
            )
...
def guard_thread(target, args: tuple, errors: list[Exception]) -> None:
    # Runs a worker loop and records any exception so run_threads can re-raise it
    try:
        target(*args)
...
def run_threads(samples: list[dict], args: argparse.Namespace) -> tuple[float, int]:
    ...
    generation_type = args.input_file_generation_type
    try:
        if generation_type == InputGenerationType.BURST:
        ...
        targets = [(metrics_collector_loop, (stop_event, container, samples, args.metrics_interval, start))]
        if generation_type == InputGenerationType.TIMED_GETFILE:
            targets.append((input_generator_loop, (stop_event, input_dir, args.input_interval, args.input_file_size)))

        threads = [
            threading.Thread(target=guard_thread, args=(target, target_args, thread_errors), daemon=True)
...
def write_results(samples: list[dict], throughput: float, flow_files_processed: int, args: argparse.Namespace) -> str:
    result = {
        "config": {
            ...
            "attribute_size_bytes": args.attribute_size,
        },
        "samples": samples,
    ...
    if output_path is None:
        ...
        name = (
            f"{timestamp}_{args.flowfile_repository}_{args.content_repository}_{args.input_file_generation_type}.json"
        )
    ...
    print(
        f"Collected {len(samples)} samples; throughput: {throughput:.2f} flow files/sec; results written to {output_path}"
    )
    return output_path
...
def main() -> None:
    ...
    parser.add_argument(
        "--flowfile-repository",
        required=True,
        choices=sorted(FLOWFILE_REPOSITORY_CLASSES),
        help="Flowfile repository type.",
    )
    parser.add_argument(
        "--content-repository",
        required=True,
        choices=sorted(CONTENT_REPOSITORY_CLASSES),
        help="Content repository type.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output", default=None, help="Output JSON path (default: results/<timestamp>_<ff>_<content>.json)."
    )
