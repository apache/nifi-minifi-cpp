...
### Flow file attributes (`--attribute-count`, `--attribute-size`)

To put extra metadata weight on each flow file (stressing the flow file
repository), `--attribute-count N` adds `N` attributes via the `UpdateAttribute`
processor, each with a random value of `--attribute-size M` bytes (accepts `64`,
`1K`, ...). Both default to `0` (no extra attributes). Example: `--attribute-count
20 --attribute-size 256` sets 20 attributes of 256 bytes on every flow file. These
flags are shared by `run_batch.py` too.
...
### Repeat runs and aggregate (`--repeat`)

A single run is noisy. Pass `--repeat N` to run each combo `N` times sequentially;
the report then aggregates the repeated runs **per combo** instead of drawing each
run separately:
...
Each run still writes its own JSON (suffixed `_runNNN`). In the report, the summary
table shows `mean ± stddev` with a `min–max` range for every metric, and the
time-series charts collapse into one mean line with a shaded min/max band per combo
(samples are aligned across runs by sample index). Runs are grouped by repository
combo, so `generate_report.py` produces the same aggregation for any set of result
files sharing a combo.
