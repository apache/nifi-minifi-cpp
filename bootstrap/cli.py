...
def run_conan_install(minifi_options: MinifiOptions, package_manager: PackageManager) -> bool:
    ...
    generator_setting = (
        " -c tools.cmake.cmaketoolchain:generator=Ninja" if minifi_options.use_ninja.value == "ON" else ""
    )
    conan_remote_add_cmd = (
        "conan remote add nifi-conan https://apache.jfrog.io/artifactory/api/conan/nifi-conan --force"
    )
...
def build_dir_menu(minifi_options: MinifiOptions, _package_manager: PackageManager) -> bool:
    questions = [
        inquirer.Path("build_dir", message="Build directory", default=minifi_options.build_dir),
    ]
    answers = inquirer.prompt(questions)
    if answers is None:
        return True
