...
int main(int argc, char** argv) {
    ...
    if (argc < 6) {
        printf("Error: must run ./tailfile_chunk <file> <interval> <chunksize> <nifi instance url> <remote port>\n");
        exit(1);
    }

    char * file = argv[1];
    char * interval = argv[2];
    char * chunksize = argv[3];
    ...
    const char * processor_name = "TailFileChunk";

    add_custom_processor(processor_name, on_trigger_tailfilechunk);
