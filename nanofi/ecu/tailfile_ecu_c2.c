...
int main(int argc, char ** argv) {
    ...
    HASH_ITER(hh, ecu_confs, el, tmp) {
      ...
      if (!out || !out->value) {
        logc(warn, "input type not provided. skip creating ecu {name: %s}. skipping...", el->name);
        continue;
      }
      ...
      HASH_FIND(hh, io_props, "type", strlen("type"), out);
      if (!out || !out->value) {
        logc(warn, "output type not provided. skip creating ecu {name: %s}. skipping...", el->name);
        continue;
      }
      ...
      if (!ip || !op) {
        logc(warn, "input/output context creation failed for ecu {name: %s}. skipping...", el->name);
        continue;
      }
