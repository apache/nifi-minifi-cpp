...
class OpenCVConan(ConanFile):
    ...
    def requirements(self):
        ...
        if self.options.get_safe("with_tiff"):
            ...
            self.requires("libtiff/4.6.0@minifi/dev") # libtiff requires xz_utils, so use minifi one
        if self.options.get_safe("with_webp"):
        ...
        if self.options.get_safe("with_ffmpeg"):
            ...
            self.requires("ffmpeg/4.4.4@minifi/dev") # ffmpeg  requires xz_utils, so use minifi one
