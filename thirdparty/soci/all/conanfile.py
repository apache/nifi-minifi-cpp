...
class SociConan(ConanFile):
    ...
    def _create_cmake_module_variables(self, module_file):
        content = textwrap.dedent(f"""\
            set(SOCI_FOUND TRUE)
            if(DEFINED SOCI_INCLUDE_DIRS)
                set(SOCI_INCLUDE_DIRS ${{SOCI_INCLUDE_DIRS}})
            endif()
            if(DEFINED SOCI_LIBRARIES)
                set(SOCI_LIBRARIES ${{SOCI_LIBRARIES}})
            endif()
            set(SOCI_VERSION_MAJOR {Version(self.version).major})
            set(SOCI_VERSION_MINOR {Version(self.version).minor})
            set(SOCI_VERSION_PATCH {Version(self.version).patch})
            set(SOCI_VERSION_STRING "{self.version}")
        """)
    ...
    def package_401_info(self):
        ...
        self.cpp_info.set_property("cmake_target_name", "SOCI::soci_core")
        self.cpp_info.set_property("cmake_build_modules", [self._module_file_rel_path])
        self.cpp_info.libs = ["soci_core"]

        # soci_core
        self.cpp_info.components["libsoci_core"].set_property("cmake_target_name", "SOCI::libsoci_core")
        if self.options.with_boost:
            self.cpp_info.components["libsoci_core"].requires.append("boost::boost")
        ...
        # soci_odbc
        if self.options.with_odbc:
            self.cpp_info.components["soci_odbc"].set_property("cmake_target_name", "SOCI::libsoci_odbc")
            self.cpp_info.components["soci_odbc"].requires = ["soci_core"]
            if self.settings.os == "Windows":
                self.cpp_info.components["soci_odbc"].system_libs.append("odbc32")
            else:
        ...
        self.cpp_info.components["soci_core"].names["cmake_find_package"] = "libsoci_core"
        self.cpp_info.components["soci_core"].names["cmake_find_package_multi"] = "libsoci_core"
        if self.options.empty:
            self.cpp_info.components["soci_empty"].names["cmake_find_package"] = "soci_empty{}".format(target_suffix)
            self.cpp_info.components["soci_empty"].names["cmake_find_package_multi"] = "soci_empty{}".format(target_suffix)
        if self.options.with_sqlite3:
            ...
            self.cpp_info.components["soci_sqlite3"].names["cmake_find_package_multi"] = "soci_sqlite3{}".format(target_suffix)
        if self.options.with_odbc:
            self.cpp_info.components["soci_odbc"].names["cmake_find_package"] = "soci_odbc"
            self.cpp_info.components["soci_odbc"].names["cmake_find_package_multi"] = "soci_odbc"
        if self.options.with_mysql:
            self.cpp_info.components["soci_mysql"].names["cmake_find_package"] = "soci_mysql{}".format(target_suffix)
            self.cpp_info.components["soci_mysql"].names["cmake_find_package_multi"] = "soci_mysql{}".format(target_suffix)
        if self.options.with_postgresql:
    ...
    def package_info(self):
        ...
        self.package_401_info()

        # self.package_403_info()
