  ...
  explicit StaticClassType(const std::string& class_name)
    ...
    MinifiProcessorClassDescription proc_description{
      ...
      .callbacks = MinifiProcessorCallbacks{
        .create = [] (MinifiProcessorMetadata metadata) -> void* {
          return new Class{core::ProcessorMetadata{
            .uuid = utils::Identifier::parse(std::string{metadata.uuid.data, metadata.uuid.length}).value(),
            .name = std::string{metadata.name.data, metadata.name.length},
            .logger = std::make_shared<core::logging::Logger>(metadata.logger)
          }};
        },
        .destroy = [] (void* self) -> void {
...
#define REGISTER_PROCESSOR(CLASSNAME) \
        static const auto& CLASSNAME##_registrar = core::StaticClassType<CLASSNAME>::get(#CLASSNAME)
