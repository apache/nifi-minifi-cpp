...
std::expected<bool, std::error_code> parseBool(const std::string_view input) {
...
std::expected<uint64_t, std::error_code> parseDataSizeMinMax(const std::string_view input, const uint64_t minimum, const uint64_t maximum) {
...
std::expected<uint64_t, std::error_code> parseDataSize(const std::string_view input) {
  return parseDataSizeMinMax(input, std::numeric_limits<uint64_t>::min(), std::numeric_limits<uint64_t>::max());
}

std::expected<uint32_t, std::error_code> parseUnixOctalPermissions(const std::string_view input) {
  uint32_t result = 0U;
  if (input.size() == 9U) {
