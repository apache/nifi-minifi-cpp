...
class MinifiController:
    ...
    def update_flow_config_through_controller(self):
        self.minifi_container.exec_run(
            [
                f"{self.bin_path}/minifi-controller",
                "--updateflow",
    ...
    def stop_component_through_controller(self, component: str):
        self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--stop", component]
        )

    def start_component_through_controller(self, component: str):
        self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--start", component]
        )

    @retry_check(max_tries=10, retry_interval_seconds=1)
    def is_component_running(self, component: str) -> bool:
        (code, output) = self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--list", "components"]
        )
        return code == 0 and component + ", running: true" in output

    def get_connections(self):
        (_, output) = self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--list", "connections"]
        )
        connections = []
    ...
    def get_full_connection_count(self) -> int:
        (_, output) = self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--getfull"]
        )
        for line in output.split("\n"):
            if "are full" in line:
    ...
    def get_connection_size(self, connection: str):
        (_, output) = self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--getsize", connection]
        )
        for line in output.split("\n"):
    ...
    def get_manifest(self) -> str:
        (_, output) = self.minifi_container.exec_run(
            [f"{self.bin_path}/minifi-controller", "--manifest"]
        )
        manifest = ""
        for line in output.split("\n"):
    ...
    def create_debug_bundle(self) -> bool:
        (code, _) = self.minifi_container.exec_run(
