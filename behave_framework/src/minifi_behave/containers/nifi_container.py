...
class NifiContainer(LinuxContainer):
    def __init__(
        ...
        super().__init__(
            "apache/nifi:" + NifiFlowDefinition.NIFI_VERSION,
            name,
        ...
        self.host_files.append(
            HostFile(
                "/scripts/convert_cert_to_jks.sh",
                os.path.join(resource_dir, "convert_cert_to_jks.sh"),
            )
        )

        nifi_client_cert, nifi_client_key = make_server_cert(
            common_name=f"nifi-{test_context.scenario_id}",
