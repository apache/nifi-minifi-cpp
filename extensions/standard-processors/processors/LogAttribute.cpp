...
void LogAttribute::onTrigger(const std::shared_ptr<core::ProcessContext> &context, const std::shared_ptr<core::ProcessSession> &session) {
  ...
  for (; i < max; ++i) {
    ...
    message << "\n" << "EntryDate:" << utils::timeutils::getTimeStr(utils::timeutils::getMillisecondsSinceUnixEpoch(flow->getEntryDate()).count());
    message << "\n" << "lineageStartDate:" << utils::timeutils::getTimeStr(utils::timeutils::getMillisecondsSinceUnixEpoch(flow->getlineageStartDate()).count());
