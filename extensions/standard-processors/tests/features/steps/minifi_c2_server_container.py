...
class MinifiC2Server(LinuxContainer):
    def __init__(self, test_context: MinifiTestContext, ssl: bool = False):
        super().__init__(
            "apache/nifi-minifi-c2:1.27.0",
            f"minifi-c2-server-{test_context.scenario_id}",
            test_context.network,
        )
        if ssl:
            c2_cert, c2_key = make_server_cert(
                f"minifi-c2-server-{test_context.scenario_id}",
            ...
            pke = jks.PrivateKeyEntry.new(
                "c2-server-cert",
                [dump_cert(c2_cert, encoding_type=serialization.Encoding.DER)],
                dump_key(c2_key, encoding_type=serialization.Encoding.DER),
                "rsa_raw",
            )
            ...
            self.files.append(
                File(
                    "/opt/minifi-c2/minifi-c2-current/certs/minifi-c2-server-keystore.jks",
            ...
            self.files.append(
                File(
                    "/opt/minifi-c2/minifi-c2-current/certs/minifi-c2-server-truststore.p12",
                    pkcs12_data,
                    permissions=0o644,
                )
            )
  ...
  """
        ...
        if ssl:
            self.host_files.append(
                HostFile(
                    "/opt/minifi-c2/minifi-c2-current/conf/authorizations.yaml",
                    os.path.join(resource_dir, "authorizations.yaml"),
                )
            )
            self.host_files.append(
                HostFile(
                    "/opt/minifi-c2/minifi-c2-current/conf/c2.properties",
