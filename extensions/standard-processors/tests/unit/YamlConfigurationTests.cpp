...
TEST_CASE("Connections components are parsed from yaml.", "[YamlConfiguration]") {
  ...
  SECTION("Source and destination names and uuids are read") {
    ...
    utils::Identifier actual_source_id = connection->getSourceUUID();
  ...
  SECTION("Errors are handled properly when configuration lines are missing") {
    ...
    SECTION("With empty configuration") {
      YAML::Node connection_node = YAML::Load(std::string(""));
      CHECK_THROWS(yaml_config.configureConnectionSourceRelationshipFromYaml(connection_node, connection));
      CHECK_NOTHROW(yaml_config.configureConnectionWorkQueueSizeFromYaml(connection_node, connection));
      CHECK_NOTHROW(yaml_config.configureConnectionWorkQueueDataSizeFromYaml(connection_node, connection));
      CHECK_THROWS(yaml_config.configureConnectionSourceUUIDFromYaml(connection_node, connection, &parent, "Test Connection"));
      CHECK_THROWS(yaml_config.configureConnectionDestinationUUIDFromYaml(connection_node, connection, &parent, "Test Connection"));
      CHECK_NOTHROW(yaml_config.configureConnectionFlowFileExpirationFromYaml(connection_node, connection));
      CHECK_NOTHROW(yaml_config.configureConnectionDropEmptyFromYaml(connection_node, connection));
    }
    SECTION("With a configuration that lists keys but has no assigned values") {
      ...
      SECTION("Single relationship name left empty") {
        ...
        CHECK_NOTHROW(yaml_config.configureConnectionSourceRelationshipFromYaml(connection_node, connection));
      }
      ...
      YAML::Node connection_node = YAML::Load(std::string {
          ...
          "flowfile expiration: \n"
          "drop empty: \n"});
      CHECK_THROWS(yaml_config.configureConnectionWorkQueueSizeFromYaml(connection_node, connection));
      CHECK_THROWS(yaml_config.configureConnectionWorkQueueDataSizeFromYaml(connection_node, connection));
      CHECK_THROWS(yaml_config.configureConnectionFlowFileExpirationFromYaml(connection_node, connection));
      CHECK_THROWS(yaml_config.configureConnectionDropEmptyFromYaml(connection_node, connection));
    }
    ...
    SECTION("Known incorrect formats that behave strangely") {
      ...
      CHECK_NOTHROW(yaml_config.configureConnectionFlowFileExpirationFromYaml(connection_node, connection));
      CHECK_THROWS(yaml_config.configureConnectionDropEmptyFromYaml(connection_node, connection));
      REQUIRE(2 == connection->getMaxQueueDataSize());
      REQUIRE(0 == connection->getFlowExpirationDuration());
    }
  }
}
