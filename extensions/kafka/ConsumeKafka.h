...
class ConsumeKafka final : public KafkaProcessorBase {
 public:
  ...
  static constexpr std::string_view KAFKA_COUNT_ATTR =
      "kafka.count";  // Always 1 until we start supporting merging from batches
  ...
  EXTENSIONAPI static constexpr auto TopicNames =
      core::PropertyDefinitionBuilder<>::createProperty("Topic Names")
          .withDescription(
              "The name of the Kafka Topic(s) to pull from. Multiple topic names are supported as a comma separated list.")
          .supportsExpressionLanguage(true)
          .isRequired(true)
          .build();
  ...
  EXTENSIONAPI static constexpr auto GroupID =
      core::PropertyDefinitionBuilder<>::createProperty("Group ID")
          .withDescription(
              "A Group ID is used to identify consumers that are within the same consumer group. Corresponds to Kafka's "
              "'group.id' property.")
          .supportsExpressionLanguage(true)
          .isRequired(true)
          .build();
  ...
  EXTENSIONAPI static constexpr auto MaxPollRecords =
      core::PropertyDefinitionBuilder<>::createProperty("Max Poll Records")
          .withDescription(
              "Specifies the maximum number of records Kafka should return when polling each time the processor is "
              "triggered.")
 ...
 private:
  ...
  std::vector<std::pair<std::string, std::string>> get_flowfile_attributes_from_message_header(
  ...
  std::optional<std::vector<std::shared_ptr<core::FlowFile>>> transform_pending_messages_into_flowfiles(
