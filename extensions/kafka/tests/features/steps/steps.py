...
def setup_publish_kafka_processor(context):
    ...
    publish_kafka.add_property(
        "Known Brokers", f"kafka-server-{context.scenario_id}:9092"
    )
...
@step(
    'a message with content "{message}" is published to the "{topic_name}" topic with key "{key}"'
)
def publish_message_with_key_to_topic(
    context: MinifiTestContext, message: str, topic_name: str, key: str
):
    kafka_server_container = context.containers["kafka-server"]
    assert isinstance(kafka_server_container, KafkaServer)
    assert (
        kafka_server_container.produce_message_with_key(
            topic_name=topic_name, message=message, message_key=key
        )
        or kafka_server_container.log_app_output()
    )
...
def set_property_to_match_message_key(
    context, property_name, processor_name, key_attribute_encoding, message_key
):
...
def publish_with_headers_to_topic(
    context, content, topic_name, semicolon_separated_headers
):
    python_code = f"""
from confluent_kafka import Producer
headers = []
...
@when(
    'two messages with content "{content_one}" and "{content_two}" is published to the "{topic_name}" topic'
)
