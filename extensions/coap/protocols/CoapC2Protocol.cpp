...
int CoapProtocol::writeHeartbeat(io::BaseStream *stream, const minifi::c2::C2Payload &payload) {
  ...
  try {
    std::string deviceIdent = minifi::c2::PayloadParser::getInstance(payload).in("deviceInfo").getAs<std::string>("identifier");

    std::string agentIdent = minifi::c2::PayloadParser::getInstance(payload).in("agentInfo").getAs<std::string>("identifier");
    /*
     std::string deviceIdent;
     // device identifier
     std::string deviceIdent = PayloadParser::getInstance(payload).get("deviceInfo").get("identifier").as<std::string>();
    ...
    /*
     ...
     auto versionedFlowSnapshotURI = getPayload("versionedFlowSnapshotURI", flowInfo);
     ...
     if (auto sub_type = std::dynamic_pointer_cast<state::response::BoolValue>(node)) {
