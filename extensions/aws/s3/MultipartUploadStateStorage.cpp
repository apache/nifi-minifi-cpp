...
std::optional<MultipartUploadState> MultipartUploadStateStorage::getState(const std::string& bucket, const std::string& key) const {
  ...
  state.upload_time_ms_since_epoch = parsing::parseIntegral<int64_t>(state_map[state_key + ".upload_time"]) | utils::orThrow(fmt::format("Expected parsable {}.upload_time", state_key));
  state.uploaded_parts = parsing::parseIntegral<size_t>(state_map[state_key + ".uploaded_parts"]) | utils::orThrow(fmt::format("Expected parsable {}.uploaded_parts", state_key));
  state.uploaded_size = parsing::parseIntegral<uint64_t>(state_map[state_key + ".uploaded_size"]) | utils::orThrow(fmt::format("Expected parsable {}.uploaded_size", state_key));
  state.part_size = parsing::parseIntegral<uint64_t>(state_map[state_key + ".part_size"]) | utils::orThrow(fmt::format("Expected parsable {}.part_size", state_key));
  state.full_size = parsing::parseIntegral<uint64_t>(state_map[state_key + ".full_size"]) | utils::orThrow(fmt::format("Expected parsable {}.full_size", state_key));
