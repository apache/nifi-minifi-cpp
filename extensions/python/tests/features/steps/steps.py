...
def build_minifi_cpp_image_with_nifi_python_processors_using_dependencies(
    ...
    # d: Delete line
    parse_document_sed_cmd = 'sed -i "/class ProcessorDetails:/,/^$/{{/^\\s*dependencies\\s*=/,/\\]\\s*$/d}}" {minifi_python_dir}/nifi_python_processors/ParseDocument.py && \\'.format(
        minifi_python_dir=minifi_python_dir_path
    )
    chunk_document_sed_cmd = 'sed -i "/class ProcessorDetails:/,/^$/{{/^\\s*dependencies\\s*=/,/\\]\\s*$/d}}" {minifi_python_dir}/nifi_python_processors/ChunkDocument.py && \\'.format(
        minifi_python_dir=minifi_python_dir_path
    )
    if python_option == PythonWithDependenciesOptions.SYSTEM_INSTALLED_PACKAGES:
        ...
        requirements_install_command = "echo 'langchain<=0.17.0' > {minifi_python_dir}/nifi_python_processors/requirements.txt && \\".format(
            minifi_python_dir=minifi_python_dir_path
        )
    elif python_option == PythonWithDependenciesOptions.INLINE_DEFINED_PACKAGES:
        parse_document_sed_cmd = (
            parse_document_sed_cmd[:-2]
            + ' sed -i "s/langchain==[0-9.]\\+/langchain<=0.17.0/" {minifi_python_dir}/nifi_python_processors/ParseDocument.py && \\'.format(
                minifi_python_dir=minifi_python_dir_path
            )
        )
        chunk_document_sed_cmd = "sed -i \"s/\\[\\'langchain\\'\\]/\\[\\'langchain<=0.17.0\\'\\]/\" {minifi_python_dir}/nifi_python_processors/ChunkDocument.py && \\".format(
            minifi_python_dir=minifi_python_dir_path
        )
