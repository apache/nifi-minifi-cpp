...
void FetchAzureDataLakeStorage::onTrigger(const std::shared_ptr<core::ProcessContext>& context, const std::shared_ptr<core::ProcessSession>& session) {
  ...
  session->write(fetched_flow_file, [&, this](const std::shared_ptr<io::BaseStream>& outputStream) -> int64_t {
    result = azure_data_lake_storage_.fetchFile(*params, *outputStream);
    if (!result) {
      return 0;
    }
