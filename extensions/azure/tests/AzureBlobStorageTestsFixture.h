...
class AzureBlobStorageTestsFixture {
 public:
  AzureBlobStorageTestsFixture() {
    ...
    auto azure_blob_storage_processor_unique_ptr = std::make_unique<core::Processor>(utils::string::partAfterLastOccurrenceOf(core::className<ProcessorType>(), ':'), impl->getName(), impl->getUUID(), std::move(impl));
