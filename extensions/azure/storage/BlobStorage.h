...
class BlobStorage {
 public:
  BlobStorage(std::string connection_string, std::string account_name, std::string endpoint_suffix, std::string container_name)
    : connection_string_(std::move(connection_string)),
      account_name_(std::move(account_name)),
      endpoint_suffix_(std::move(endpoint_suffix)),
  ...
  virtual void resetClientIfNeeded(const ManagedIdentityParameters &managed_identity_params, const std::string &container_name) = 0;
