...
class OpensearchContainer(ElasticBaseContainer):
    ...
    def add_elastic_user_to_opensearch(self):
        (code, output) = self.exec_run(["/bin/bash", "-c",
                                        f'curl -s -u admin:admin -k -XPUT https://{self.container_name}:9200/_plugins/_security/api/internalusers/elastic -H Content-Type:application/json -d\'{{"password":"password","backend_roles":["admin"]}}\''])
