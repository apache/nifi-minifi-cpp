...
def setup_odbc_service_for_processor(
    context: MinifiTestContext, processor_name: str, service_name: str
):
...
@then(
    'the query "{query}" returns {rows:d} rows in less than {timeout_str} on the PostgreSQL server'
)
