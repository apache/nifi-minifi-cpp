...
void testSuccessfulFiltering(const std::function<bool(const std::string&, const std::string&, const std::string&)>& filter, const std::string& expected_result) {
...
TEST_CASE("Without filtering, we get the full result back", "[kubernetes][metrics_filter]") {
  const auto always_true = [](const std::string&, const std::string&, const std::string&) { return true; };
...
TEST_CASE("We can filter to get metrics for the default namespace only", "[kubernetes][metrics_filter]") {
  const auto default_namespace_only = [](const std::string& name_space, const std::string&, const std::string&) { return name_space == "default"; };
...
TEST_CASE("We can filter to get metrics for a selected pod only", "[kubernetes][metrics_filter]") {
  const auto minifi_pod_only = [](const std::string&, const std::string& pod_name, const std::string&) { return pod_name == "minifi"; };
...
TEST_CASE("We can filter to get metrics for a selected container only", "[kubernetes][metrics_filter]") {
  const auto one_container_only = [](const std::string&, const std::string& pod_name, const std::string& container_name) { return pod_name == "hello-world" && container_name == "echo-two"; };
...
TEST_CASE("We can filter to get no metrics (not super useful)", "[kubernetes][metrics_filter]") {
  const auto always_false = [](const std::string&, const std::string&, const std::string&) { return false; };
