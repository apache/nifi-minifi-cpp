...
Feature: JoinEnrichmentAttributes recovers released FlowFiles after an agent restart

  # JoinEnrichmentAttributes releases the first half of a pair and holds it in memory until the
  ...
  # ForkEnrichment) so their arrival can be separated across the restart: the ORIGINAL is present
  ...
  Scenario: A released FlowFile held across an agent restart is recovered and joined with its late-arriving pair
    ...
    And the "success" relationship of the TagOriginal processor is connected to the JoinEnrichmentAttributes
    And the "success" relationship of the GetEnrichment processor is connected to the TagEnrichment
    And the "success" relationship of the TagEnrichment processor is connected to the JoinEnrichmentAttributes
    And the "joined" relationship of the JoinEnrichmentAttributes processor is connected to the PutFile

    And JoinEnrichmentAttributes's original relationship is auto-terminated
    And JoinEnrichmentAttributes's invalid relationship is auto-terminated
    And JoinEnrichmentAttributes's timeout relationship is auto-terminated
    ...
    # JoinEnrichmentAttributes gets the ORIGINAL, releases it and holds it waiting for its pair,
    # so nothing is joined yet.
    Then no files are placed in the "/tmp/output" directory in 5 seconds of running time

    # Graceful stop destroys JoinEnrichmentAttributes while it still holds the released ORIGINAL,
