...
impl SDK {
    fn from_local_repo(repo_root: &Path) -> Option<Self> {
        ...
        let def_path = canon.to_path_buf().join("minifi-api/minifi-c-api.def");
    ...
    fn from_local_sdk_path(sdk_path: &Path) -> Option<Self> {
        let base_path = if sdk_path.join("minifi-c.h").exists() {
            sdk_path.to_path_buf()
        } else if sdk_path.join("minifi-native-sdk/minifi-c.h").exists() {
