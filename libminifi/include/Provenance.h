...
class ProvenanceReporter {
 ...
 public:
  ...
  //! route
  void route(FlowFileRecord *flow, Relationship relation, std::string detail,
             uint64_t processingDuration);
  //! modifyAttributes
  void modifyAttributes(FlowFileRecord *flow, std::string detail);
  //! modifyContent
  void modifyContent(FlowFileRecord *flow, std::string detail,
                     uint64_t processingDuration);
  //! clone
  ...
  //! send
  void send(FlowFileRecord *flow, std::string transitUri, std::string detail,
            uint64_t processingDuration, bool force);
  //! fetch
  void fetch(FlowFileRecord *flow, std::string transitUri, std::string detail,
             uint64_t processingDuration);
  //! receive
  void receive(FlowFileRecord *flow, std::string transitUri,
  ...
  virtual bool initialize() {
    ...
    if (configure_->get(Configure::nifi_provenance_repository_max_storage_time,
      ...
      if (Property::StringToTime(value, _maxPartitionMillis, unit)
          && Property::ConvertTimeUnitToMS(_maxPartitionMillis, unit,
                                           _maxPartitionMillis)) {
      }
    }
    ...
    leveldb::Status status = leveldb::DB::Open(options, _directory.c_str(),
                                               &_db);
    if (status.ok()) {
