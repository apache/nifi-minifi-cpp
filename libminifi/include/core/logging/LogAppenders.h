...
class OutputStreamAppender : public BaseLogger {
 ...
 public:
   ...
   * @param config configuration.
   */
  OutputStreamAppender(std::ostream &stream, const std::shared_ptr<Configure> &config)
      : BaseLogger("info") {
    auto ostream_sink = std::make_shared<spdlog::sinks::ostream_sink_mt>(
        stream);
    std::string unique_name = getUniqueName<OutputStreamAppender>();
...
class LogInstance {
 public:
   ...
   */
  static std::unique_ptr<BaseLogger> getConfiguredLogger(const std::shared_ptr<Configure> &config) {
    std::string appender = "";
