  ...
  if (record) {
    if (parent->_claim) {
      if ((offset + size) > (long) parent->_size) {
        ...
        std::map<std::string, FlowFileRecord *>::iterator it = this
            ->_addedFlowFiles.find(record->getUUIDStr());
...
void ProcessSession::write(FlowFileRecord *flow,
...
void ProcessSession::append(FlowFileRecord *flow,
                            OutputStreamCallback *callback) {
  ResourceClaim *claim = NULL;
    ...
    for (auto && it : _updatedFlowFiles) {
      ...
      if (itRelationship != _transferRelationship.end()) {
        ...
        std::set<Connection *> connections = _processContext->getProcessor()
            ->getOutGoingConnections(relationship.getName());
        if (connections.empty()) {
          // No connection
          if (!_processContext->getProcessor()->isAutoTerminated(
              relationship)) {
...
FlowFileRecord *ProcessSession::get() {
  ...
  do {
    ...
    if (expired.size() > 0) {
      ...
      for (std::set<FlowFileRecord *>::iterator it = expired.begin();
          it != expired.end(); ++it) {
