...
asio::ssl::context setupSslContext(SslServerOptions& ssl_data) {
  ...
  ssl_context.use_certificate_file(ssl_data.cert_data.cert_loc, asio::ssl::context::pem);
  ssl_context.use_private_key_file(ssl_data.cert_data.key_loc, asio::ssl::context::pem);
  ssl_context.load_verify_file(ssl_data.cert_data.ca_loc);
