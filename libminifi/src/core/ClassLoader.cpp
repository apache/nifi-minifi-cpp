...
class ProcessorFactoryWrapper : public ObjectFactoryImpl {
 public:
  ...
  [[nodiscard]] gsl::owner<CoreComponent*> createRaw(const std::string &name, const utils::Identifier &uuid) override {
    ...
    return gsl::owner<CoreComponent*>{new Processor(utils::string::partAfterLastOccurrenceOf(factory_->getClassName(), ':'), name, uuid, factory_->create({.uuid = uuid, .name = name, .logger = std::move(logger)}))};
  }
