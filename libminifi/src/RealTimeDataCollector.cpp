...
Property RealTimeDataCollector::FILENAME(
...
Property RealTimeDataCollector::REALTIMESERVERPORT("Real Time Server Port",
                                                   "Real Time Server Port",
                                                   "10000");
Property RealTimeDataCollector::BATCHSERVERNAME("Batch Server Name",
                                                "Batch Server Name",
                                                "localhost");
Property RealTimeDataCollector::BATCHSERVERPORT("Batch Server Port",
                                                "Batch Server Port", "10001");
Property RealTimeDataCollector::ITERATION(
    "Iteration", "If true, sample osp file will be iterated", "true");
Property RealTimeDataCollector::REALTIMEMSGID("Real Time Message ID",
                                              "Real Time Message ID", "41");
Property RealTimeDataCollector::BATCHMSGID("Batch Message ID",
                                           "Batch Message ID", "172, 30, 48");
Property RealTimeDataCollector::REALTIMEINTERVAL(
    "Real Time Interval", "Real Time Data Collection Interval in msec",
    "10 ms");
Property RealTimeDataCollector::BATCHINTERVAL(
    "Batch Time Interval", "Batch Processing Interval in msec", "100 ms");
Property RealTimeDataCollector::BATCHMAXBUFFERSIZE(
    "Batch Max Buffer Size", "Batch Buffer Maximum size in bytes", "262144");
Relationship RealTimeDataCollector::Success(
...
void RealTimeDataCollector::onTrigger(ProcessContext *context,
                                      ProcessSession *session) {
    ...
    if (!this->_firstInvoking) {
      ...
      if (this->getProperty(REALTIMEINTERVAL.getName(), value)) {
        ...
        if (Property::StringToTime(value, _realTimeInterval, unit)
            && Property::ConvertTimeUnitToNS(_realTimeInterval, unit,
                                             _realTimeInterval)) {
      ...
      if (this->getProperty(BATCHINTERVAL.getName(), value)) {
        ...
        if (Property::StringToTime(value, _batchInterval, unit)
            && Property::ConvertTimeUnitToNS(_batchInterval, unit,
                                             _batchInterval)) {
