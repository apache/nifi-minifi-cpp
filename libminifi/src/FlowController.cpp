  ...
  if (processorsNode) {
    ...
    if (processorsNode.IsSequence()) {
      ...
      for (YAML::const_iterator iter = processorsNode.begin();
        ...
        TimeUnit unit;
        if (Property::StringToTime(procCfg.schedulingPeriod, schedulingPeriod,
                                   unit)
...
void FlowControllerImpl::parseRemoteProcessGroupYaml(
    YAML::Node *rpgNode, ProcessGroup *parentGroup) {
  uuid_t uuid;
    ...
    if (rpgNode->IsSequence()) {
      for (YAML::const_iterator iter = rpgNode->begin(); iter != rpgNode->end();
        ...
        if (Property::StringToTime(timeout, timeoutValue, unit)
            && Property::ConvertTimeUnitToMS(timeoutValue, unit, timeoutValue)
            && group) {
          logger_->log_debug(
