...
void Decryptor::decryptSensitiveProperties(Configure& configure) const {
  ...
  for (const auto& property_key : configure.getConfiguredKeys()) {
    ...
    utils::optional<std::string> encryption_type = configure.get(property_key + ".protected");
    if (Decryptor::isEncrypted(encryption_type)) {
      std::string decrypted_property_value;
      try {
        decrypted_property_value = decrypt(utils::StringUtils::from_base64(property_value), property_key);
