...
void C2Client::initialize(core::controller::ControllerServiceProvider *controller, state::PausableStateController *state_controller, const std::shared_ptr<state::StateMonitor> &update_sink) {
  if (!isC2Enabled()) {
    return;
  }
  ...
  if (!initialized_) {
    ...
    c2_agent_ = std::unique_ptr<c2::C2Agent>(new c2::C2Agent(controller, state_controller, update_sink, configuration_, filesystem_));
    c2_agent_->start();
    initialized_ = true;
  }
}
