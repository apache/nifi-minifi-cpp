...
struct VerifiedQueue {
  ...
  VerifiedQueue(std::shared_ptr<minifi::SwapManager> swap_manager, std::shared_ptr<utils::timeutils::SteadyClock> clock)
    : impl(std::move(swap_manager)),
      clock_(std::move(clock)) {
    FlowFileQueueTestAccessor::get_clock_(impl) = clock_;
  }

  utils::FlowFileQueue impl;
  FlowFilePtrVec ref_;
  std::shared_ptr<utils::timeutils::SteadyClock> clock_;
};
...
class SwapTestController : public TestController {
 public:
  SwapTestController() {
    ...
    clock_ = std::make_shared<utils::ManualSteadyClock>();
    queue_ = std::make_shared<VerifiedQueue>(
        std::static_pointer_cast<minifi::SwapManager>(flow_repo_),
            std::static_pointer_cast<utils::timeutils::SteadyClock>(clock_));
  }
  ...
  std::shared_ptr<utils::ManualSteadyClock> clock_;
};
