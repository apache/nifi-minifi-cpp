...
TEST_CASE("net::reverseLookup", "[net][dns][reverseLookup]") {
  SECTION("dns.google IPv4") {
    auto dns_google_hostname = net::reverseLookup(asio::ip::address::from_string("8.8.8.8"));
  ...
  SECTION("dns.google IPv6") {
    if (minifi::test::utils::isIPv6Disabled())
      return;
    auto dns_google_hostname = net::reverseLookup(asio::ip::address::from_string("2001:4860:4860::8888"));
    REQUIRE(dns_google_hostname.has_value());
    CHECK(dns_google_hostname == "dns.google");
  }

  SECTION("Unresolvable address IPv4") {
    auto unresolvable_hostname = net::reverseLookup(asio::ip::address::from_string("192.0.2.0"));
  ...
  SECTION("Unresolvable address IPv6") {
    if (minifi::test::utils::isIPv6Disabled())
      return;
    auto unresolvable_hostname = net::reverseLookup(asio::ip::address::from_string("2001:db8::"));
    REQUIRE(unresolvable_hostname.has_value());
